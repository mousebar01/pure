...
const RPC_IMPLEMENTATION_VERSION = 2;
...
export class AgentSessionWrapper {
  ...
  start(): void {
    this.unsubscribe = this.inner.subscribe((event: AgentEvent) => {
      // agent_end covers normal runs; extension-injected runs settle via
      // agent_settled and may never emit agent_end. Invalidate on both so a
      // freshly flushed session file is visible to the sidebar immediately.
  ...
  beginExtensionBinding(options: ExtensionBindingOptions = {}): void {
    void this.ensureExtensionsBound(options).catch((err) => {
      console.error("[pure] failed to dispatch session_start to extensions:", err instanceof Error ? err.message : err);
    });
  }
  ...
  private ensureExtensionsBound(options: ExtensionBindingOptions = {}): Promise<void> {
    ...
    this.extensionBindingPromise = (async () => {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in pure.",
      ...
      this.applyForcedEmptySystemPrompt();
      console.log(`[pure] session_start dispatched to extensions for session ${this.inner.sessionId}`);
    })().catch((err) => {
  ...
  private resetIdleTimer(): void {
    ...
    this.idleTimer = setTimeout(() => {
      ...
      void this.shutdown().catch((error) => {
        console.error("[pure] failed to shut down idle session:", error instanceof Error ? error.message : error);
      });
    }, 10 * 60 * 1000);
  }
    ...
    switch (type) {
      case "prompt": {
        ...
        this.inner.prompt(command.message as string, {
          ...
          this.resetIdleTimer();
          // The SDK flushes a brand-new session's file once the first
          // assistant message exists. Invalidate the cached list so the
          // client-side reload after prompt_done returns the new session.
          invalidateSessionListCache();
      ...
      case "fork": {
        ...
        if (!entry.parentId) {
          ...
          // Fork after a completed answer when requested by the answer action;
          // legacy callers can still fork before the selected entry.
          const sourceManager = SessionManager.open(currentSessionFile, sessionDir);
          const forkLeafId = command.includeEntry === true ? entry.id : entry.parentId;
          const forkedPath = sourceManager.createBranchedSession(forkLeafId);
    ...
    this.shutdownPromise = (async () => {
      try {
        try {
          ...
          console.error(
            "[pure] extension binding failed before session shutdown:",
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in pure extension UI yet" }),
...
declare global {
  ...
  var __piRpcImplementationVersion: number | undefined;
  var __piRegistryCleanupRegistered: boolean | undefined;
}

function getRegistry(): Map<string, AgentSessionWrapper> {
  if (globalThis.__piRpcImplementationVersion !== RPC_IMPLEMENTATION_VERSION) {
    ...
    globalThis.__piRpcImplementationVersion = RPC_IMPLEMENTATION_VERSION;
  }
  if (!globalThis.__piSessions) {
    globalThis.__piSessions = new Map();
  }
  if (!globalThis.__piRegistryCleanupRegistered) {
    const cleanup = () => globalThis.__piSessions?.forEach((s) => s.destroy());
    process.once("exit", cleanup);
    process.once("SIGINT", cleanup);
    process.once("SIGTERM", cleanup);
    globalThis.__piRegistryCleanupRegistered = true;
  }
...
export async function shutdownAllRpcSessions(): Promise<number> {
  const sessions = Array.from(new Set(getRegistry().values()));
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    if (toolNames !== undefined) {
      ...
      // tool registry — so they were unavailable in pure sessions even though the
    ...
    // extensions stay usable in pure just like in the `pi` CLI.
