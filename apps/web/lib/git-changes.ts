...
export async function getGitFileDiff(cwd: string, filePath: string): Promise<GitFileDiffResponse> {
  let repositoryRoot = await findRepositoryRoot(cwd);
  // Files opened from the sidebar (search hit / changes list) may live in a
  // different project than the selected session's cwd — e.g. browsing repo B
  // while chatting in repo A. Fall back to the file's own repository when
  // the cwd's repo doesn't contain the file (or cwd isn't a repo at all).
  if (!repositoryRoot || !isWithinPath(repositoryRoot, filePath)) {
    repositoryRoot = await findRepositoryRoot(path.dirname(filePath));
  }
